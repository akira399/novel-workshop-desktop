 ...
 */
import type { CommandMap, CommandName, CommandRequest, CommandResponse, StreamChunk } from '@dafuyu/contracts'
...
/** 订阅主进程流式分片；返回取消订阅函数。 */
export function onStream(listener: (chunk: StreamChunk) => void): () => void {
  return window.novelWorkshop.on('stream', listener)
}
